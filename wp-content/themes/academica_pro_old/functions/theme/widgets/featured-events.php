...
class wpzoom_widget_feat_events extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		if ($time_format == '24h') {
			$time_start = date("H:i", mktime($event_start_hour,$event_start_minute,0,$event_start_month, $event_start_day, $event_start_year));
			$time_end = date("H:i", mktime($event_end_hour,$event_end_minute,0,$event_end_month, $event_end_day, $event_end_year));
		} elseif ($time_format == '12h1') {
			$time_start = date("g:i a", mktime($event_start_hour,$event_start_minute,0,$event_start_month, $event_start_day, $event_start_year));
			$time_end = date("g:i a", mktime($event_end_hour,$event_end_minute,0,$event_end_month, $event_end_day, $event_end_year));
		} else {
			$time_start = date("g:i A", mktime($event_start_hour,$event_start_minute,0,$event_start_month, $event_start_day, $event_start_year));
			$time_end = date("g:i A", mktime($event_end_hour,$event_end_minute,0,$event_end_month, $event_end_day, $event_end_year));
		}
			...
			<li>
				...
				<div class="post-excerpt">
					...
					<span class="meta-date">
						...
						<span class="value value-time"> @ <?php echo "$time_start - $time_end"; ?></span>
						<?php } ?>
					</span><!-- .meta-date -->
					<?php } ?>
		...
		function form( $instance ) {
			...
			$defaults = array('title' => 'Widget Title','num' => '3','time_format' => '24h');
	    ...
	    ?>
			...
			<p>
				...
				<select id="<?php echo $this->get_field_id( 'time_format' ); ?>" name="<?php echo $this->get_field_name( 'time_format' ); ?>" style="width:90%;">
					<option value="24h"<?php if ($instance['time_format'] == '24h') { echo ' selected="selected"';} ?>>15:45 - 18:45</option>
					<option value="12h1"<?php if ($instance['time_format'] == '12h1') { echo ' selected="selected"';} ?>>3:45 pm - 6:45 pm</option>
					<option value="12h2"<?php if ($instance['time_format'] == '12h2') { echo ' selected="selected"';} ?>>3:45 PM - 6:45 PM</option>
				</select>
			</p>
