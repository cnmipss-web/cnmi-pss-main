...
class WPZOOM_Widget_Text extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		if (isset($instance['widget_style'])) { $widget_style = $instance['widget_style']; }
		
		$pos = strpos($before_widget,'widget ');
		if (isset($pos) && isset($widget_style)) { $before_widget=substr_replace($before_widget, 'widget-' . $widget_style . ' ', $pos, 0); }
...
?>
		...
		<p>
			...
			<select id="<?php echo $this->get_field_id( 'widget_style' ); ?>" name="<?php echo $this->get_field_name( 'widget_style' ); ?>">
				...
				<option value="red"<?php if ($instance['widget_style'] == 'red') { echo ' selected="selected"';} ?>>Red</option>
			</select>
		</p>
