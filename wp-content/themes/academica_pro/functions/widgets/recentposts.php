...
class Wpzoom_Feature_Posts extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		?>
			<li>
				<?php
				get_the_image( array( 'size' => 'loop-main', 'width' => 250, 'height' => 140, 'before' => '<div class="post-cover">', 'after' => '</div>' ) );
				?>
				...
				<div class="post-excerpt">
					...
					<p class="post-meta">
						<span>
							<?php if ($showDate == 'on') { ?><time datetime="<?php the_time("Y-m-d"); ?>" pubdate><?php printf( __('%s', 'wpzoom'),  get_the_date()); ?></time><?php $prev = TRUE; } ?>
	...
	function form( $instance ) {
		...
		<p>
			...
			<select id="<?php echo $this->get_field_id( 'widget_style' ); ?>" name="<?php echo $this->get_field_name( 'widget_style' ); ?>">
				<option value="0"<?php if (!isset($instance['widget_style']) || $instance['widget_style'] == '0') { echo ' selected="selected"';} ?>>Basic</option>
				<option value="blue"<?php if (isset($instance['widget_style']) && $instance['widget_style'] == 'blue') { echo ' selected="selected"';} ?>>Blue</option>
				<option value="gold"<?php if (isset($instance['widget_style']) && $instance['widget_style'] == 'gold') { echo ' selected="selected"';} ?>>Gold</option>
				<option value="grey"<?php if (isset($instance['widget_style']) && $instance['widget_style'] == 'grey') { echo ' selected="selected"';} ?>>Grey</option>
				<option value="red"<?php if (isset($instance['widget_style']) && $instance['widget_style'] == 'red') { echo ' selected="selected"';} ?>>Red</option>
			</select>
		</p>
