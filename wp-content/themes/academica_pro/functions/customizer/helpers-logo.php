...
function academica_pro_logo() {
    $info = academica_pro_get_logo_information();
    ...
    if ( get_theme_mod( 'logo-retina-ready', academica_pro_get_default( 'logo-retina-ready' ) ) ) {
        $width /= 2;
        $height /= 2;
    }
 ...
 */
function academica_pro_has_logo() {
    $information = academica_pro_get_logo_information();
 ...
 */
function academica_pro_get_logo_information( $force = false ) {
    $logo_information = array();

    $logo_information['image'] = get_theme_mod( 'logo', academica_pro_get_default( 'logo' ) );

    if ( ! empty( $logo_information['image'] ) ) {
        $dimensions = academica_pro_get_logo_dimensions( $logo_information['image'], $force );
...
function academica_pro_get_logo_dimensions( $url, $force = false ) {
    // Build the cache key
    $key = 'angle-' . md5( 'logo-dimensions-' . $url . WPZOOM::$themeVersion );
    ...
    if ( false === $dimensions || is_preview() || true === $force ) {
        ...
        $attachment_id = academica_pro_get_attachment_id_from_url( $url );
 ...
 */
function academica_pro_get_attachment_id_from_url( $url = '' ) {
 ...
 * @return void
 */
function academica_pro_display_favicons() {
    $logo_favicon = get_theme_mod( 'logo-favicon', academica_pro_get_default( 'logo-favicon' ) );
    if ( ! empty( $logo_favicon ) ) : ?>
        <link rel="icon" href="<?php echo esc_url( $logo_favicon ); ?>"/>
    <?php endif;
}

add_action( 'wp_head', 'academica_pro_display_favicons' );
